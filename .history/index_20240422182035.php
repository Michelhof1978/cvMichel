...
<script>
  ...
  if (!isPopupShown()) {
    ...
    // Style du popup ajusté pour les mobiles à une taille réduite de 50%
    popupDiv.style.cssText = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; text-align: center; background: white; border: 1px solid #ccc; border-radius: 8px; padding: 10px; width: ${window.innerWidth < 600 ? '40%' : 'auto'};`;

    let img = document.createElement("img");
    img.src = "../images/diplome.jpg";
    img.style.height = window.innerWidth < 600 ? '100px' : '75%'; // Hauteur réduite à 100px sur mobile
    img.style.width = 'auto'; // Largeur ajustée automatiquement pour maintenir le ratio d'aspect
    popupDiv.appendChild(img);

    let closeButton = document.createElement("button");
    closeButton.textContent = "X";
    // Bouton de fermeture ajusté pour les mobiles
    closeButton.style.cssText = `position: absolute; top: 5px; right: 5px; cursor: pointer; border: none; background: transparent; font-size: ${window.innerWidth < 600 ? '16px' : '20px'};`;
