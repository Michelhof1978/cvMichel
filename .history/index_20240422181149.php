...
<script>
  ...
  if (!isPopupShown()) {
    ...
    // Style du popup, avec des ajustements pour les mobiles
    popupDiv.style.cssText = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; text-align: center; background: white; border: 1px solid #ccc; border-radius: 8px; padding: ${window.innerWidth < 600 ? '40px' : '25px'}; width: ${window.innerWidth < 600 ? '90%' : 'auto'};`;

    let img = document.createElement("img");
    img.src = "../images/diplome.jpg"; 
    img.style.height = window.innerWidth < 600 ? '80%' : '75%'; // Taille de l'image ajustée pour mobile
    img.style.width = 'auto'; // Largeur automatique pour garder le ratio d'aspect
    popupDiv.appendChild(img);

    let closeButton = document.createElement("button");
    closeButton.textContent = "X";
    // Style du bouton de fermeture, agrandi pour les mobiles
    closeButton.style.cssText = `position: absolute; top: 10px; right: 10px; cursor: pointer; border: none; background: transparent; font-size: ${window.innerWidth < 600 ? '24px' : '20px'};`;
