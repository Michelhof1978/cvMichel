...
<script>
  ...
  if (!isPopupShown()) {
    ...
    // Style du popup ajusté pour les mobiles
    popupDiv.style.cssText = `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); z-index: 9999; text-align: center; background: white; border: 1px solid #ccc; border-radius: 8px; padding: ${window.innerWidth < 600 ? '20px' : '25px'}; width: ${window.innerWidth < 600 ? '80%' : 'auto'};`;

    let img = document.createElement("img");
    img.src = "../images/diplome.jpg"; 
    img.style.height = window.innerWidth < 600 ? '50%' : '75%'; // Taille de l'image ajustée pour mobile
    img.style.width = 'auto'; // Largeur automatique pour garder le ratio d'aspect
    popupDiv.appendChild(img);

    let closeButton = document.createElement("button");
    closeButton.textContent = "X";
    // Style du bouton de fermeture, ajusté pour les mobiles
    closeButton.style.cssText = `position: absolute; top: 10px; right: 10px; cursor: pointer; border: none; background: transparent; font-size: ${window.innerWidth < 600 ? '20px' : '20px'};`;
